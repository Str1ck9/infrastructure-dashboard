(function () {
  ...
  // Realistic base metrics that fluctuate slightly
  let baseMetrics = {
    cpu: 42,
    memory: 65,
    pool: 58,
  ...
  // Slight fluctuation around base values for realism
  function fluctuate(base, range = 3, precision = 0) {
    const factor = Math.pow(10, precision);
  ...
  function updateMetrics() {
    // Use fluctuation for more realistic changes
    const cpu = fluctuate(baseMetrics.cpu, 4);
    const memory = fluctuate(baseMetrics.memory, 2);
    const pool = fluctuate(baseMetrics.pool, 1);
    const network = fluctuate(baseMetrics.network, 1.5, 1);

    // Update base occasionally for drift
    if (Math.random() < 0.1) {
      baseMetrics.cpu = Math.max(30, Math.min(70, baseMetrics.cpu + (Math.random() - 0.5) * 3));
      baseMetrics.memory = Math.max(55, Math.min(75, baseMetrics.memory + (Math.random() - 0.5) * 2));
      baseMetrics.network = Math.max(5, Math.min(15, baseMetrics.network + (Math.random() - 0.5)));
    }
  ...
  function populatePools() {
    // More realistic pool capacity with TiB calculation
    const tankPercent = 58.3;
    const backupPercent = 44.7;
    const tankUsed = (240 * tankPercent / 100).toFixed(1);
    const backupUsed = (120 * backupPercent / 100).toFixed(1);
    
    $('#tankUsage').textContent = `${tankUsed} TiB (${tankPercent}%)`;
    $('#backupUsage').textContent = `${backupUsed} TiB (${backupPercent}%)`;
  }
